...
const Cadenasdecabeza = () => {
  ...
  useEffect(() => {
    async function getData() {
        const response = await fetch("products/cadenasdecabeza.csv");
  ...
  return (
    <>
      <Breadcrumb
        pageName="Cadenas de Cabeza"
        description="Accesorios únicos, perfectos para eventos especiales con un estilo romántico y elegante."
      />
                ...
                parsedCsvData.map((parsedData, index) => (
                  <tr key={index}>
                    <td className="px-4 py-3 border-b border-gray-200 dark:border-gray-600">
                      <Image
                        src={`/images/products/cadenasdecabeza/${parsedData.imagen}.1.webp`} // Use template literal for clarity
                        width={100}
                        height={100}
                        alt={`Product Image for ${parsedData.item}`} // Dynamic alt text
                        className="w-40 h-40 object-cover cursor-pointer"
                        onClick={() => handleImageClick(`/images/products/cadenasdecabeza/${parsedData.imagen}.1.webp`)}
                      />
...
export default Cadenasdecabeza;
