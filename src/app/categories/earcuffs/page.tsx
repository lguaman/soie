...
const Earcuffs = () => {
  ...
  useEffect(() => {
    async function getData() {
        const response = await fetch("products/earcuffs.csv");
  ...
  return (
    <>
      <Breadcrumb
        pageName="Ear Cuffs"
        description="Adornos para la oreja sin necesidad de perforaciones. Diseños modernos e innovadores que complementan cualquier look."
      />
                ...
                parsedCsvData.map((parsedData, index) => (
                  <tr key={index}>
                    <td className="px-4 py-3 border-b border-gray-200 dark:border-gray-600">
                      <Image
                        src={`/images/products/earcuffs/${parsedData.imagen}.1.webp`} // Use template literal for clarity
                        width={100}
                        height={100}
                        alt={`Product Image for ${parsedData.item}`} // Dynamic alt text
                        className="w-40 h-40 object-cover cursor-pointer"
                        onClick={() => handleImageClick(`/images/products/earcuffs/${parsedData.imagen}.1.webp`)}
                      />
